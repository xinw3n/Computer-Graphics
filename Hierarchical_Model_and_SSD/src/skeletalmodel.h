...
public:
    ...
    void traverseJoints(Joint* root, MatrixStack& stack, const Camera& camera);
    ...
    void traverseSkeleton(Joint* root, MatrixStack& stack, const Camera& camera);
    ...
    void traverseBindWorldToJoint(Joint* root, MatrixStack& stack);

    // 2.3.2. Implement this method to compute a per-joint transform from
    // joint space to world space in the CURRENT POSE.
    void updateCurrentJointToWorldTransforms();
    void traverseCurrentJointToWorld(Joint* root, MatrixStack& stack);
