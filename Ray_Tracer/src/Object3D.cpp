...
Plane::Plane(const Vector3f &normal, float d, Material *m) : Object3D(m) {
    // DONE implement Plane constructor
    _normal = normal;
    _offset = d;
}
...
{
    // Locate intersection point ( 2 pts )
    const Vector3f &origin = r.getOrigin(); //Ray origin in the world coordinate
    const Vector3f &dir = r.getDirection();
    
    float denominator = Vector3f::dot(_normal, dir);
    // ray is parallel with the plane --> no intersection
    if (denominator == 0){
        return false;
    }
    
    float t = (_offset - Vector3f::dot(_normal, origin)) / denominator;
